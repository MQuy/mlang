...
public:
	...
	llvm::Value *load_value(llvm::Value *source, const std::shared_ptr<ExprAST> &expr);
	llvm::Value *cast_value(llvm::Value *source, const std::shared_ptr<TypeAST> &src_type_ast, const std::shared_ptr<TypeAST> &dest_type_ast);
	llvm::Value *convert_to_bool(llvm::Value *source, std::string name);
	llvm::Type *get_type(const std::shared_ptr<TypeAST> &type_ast);
	...
	std::vector<int> build_indices(const std::shared_ptr<AggregateTypeAST> &type_ast, std::string member_name);
	std::vector<llvm::Value *> get_indices(const std::shared_ptr<AggregateTypeAST> &type_ast, std::string member_name);
	void complete_block(llvm::Function *func, const std::shared_ptr<ASTNode> &node, llvm::BasicBlock *nextbb);
	void branch_block(llvm::Function *func, const std::shared_ptr<ExprAST> &expr, llvm::BasicBlock *truebb, llvm::BasicBlock *falsebb);
	...
	void unwind_stmt_branch(const std::shared_ptr<StmtBranch> &block, bool self_included = true);
	void enter_scope();
	void leave_scope();
	void calculate_array_type_size(std::shared_ptr<TypeAST> &type_ast, const std::shared_ptr<ExprAST> &expr);
	llvm::Value *execute_binop(BinaryOperator op, const std::shared_ptr<TypeAST> &type_ast, llvm::Value *left, llvm::Value *right);
	llvm::Function *create_function_prototype(std::string name, const std::shared_ptr<TypeAST> &type_ast);
	llvm::Value *get_or_insert_global_string(std::string content);
	unsigned get_sizeof_type(const std::shared_ptr<TypeAST> &type_ast);
	unsigned get_alignof_type(const std::shared_ptr<TypeAST> &type_ast);
	void store_inst(llvm::Value *dest, const std::shared_ptr<TypeAST> &dest_type, llvm::Value *src, const std::shared_ptr<TypeAST> &src_type);
	BinaryOperator convert_assignment_to_arithmetic_binop(BinaryOperator binop);
	std::string get_aggregate_name(const std::shared_ptr<TypeAST> &type_ast);
	llvm::Value *build_aggregate_accesses(llvm::Value *object, const std::shared_ptr<AggregateTypeAST> &type_ast, std::vector<int> indices);
	std::shared_ptr<TypeAST> get_largest_aggregate_member(const std::shared_ptr<AggregateTypeAST> &type_ast);
	...
	std::vector<llvm::Type *> get_aggregate_member_types(const std::shared_ptr<AggregateTypeAST> &type_ast);
